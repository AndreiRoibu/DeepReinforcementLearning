# DQN for Breakout (version 1.0)
# ------------------------------
# Code written by Andrei Roibu
# Created: 21/02/2019
# Last Modified: 25/03/2019
# ------------------------------
# This script looks at implementing the DQN algorithm for solving
...
# the DSN, DQN-DFA and DSN-DFA codes.
...
# We now start coding the two types of layers comprising the DQN:
...
class DQN:
    ...
    def save(self):
        ...
        np.savez('dqn_breakout_tensorflow_weights.npz', *parameters)

    def load(self):
        parameters = [variable for variable in tf.trainable_variables() if variable.name.startswith(self.scope)]
        npz = np.load('dqn_breakout_tensorflow_weights.npz')
        operations = []
...
def learn(network, target_network, experience_replay, gamma, batch_size):
    ...
    Q_predicted = np.amax(Q_values, axis= 1)
...
def play_one_episode(environment, session, total_steps, experience_replay, network, target_network, image_transformer, gamma, batch_size, epsilon, epsilon_change, epsilon_minimum):
    ...
    while not done:
        if total_steps % copy_period == 0:
            ...
            print("Copied network parameters to target network. total steps = %s, copy period = %s" % (total_steps, copy_period),file=open("output2.txt", "a"))

        action = network.sample_action(state, epsilon)
        ...
        loss = learn(network, target_network, experience_replay, gamma, batch_size)
...
def main():
    ...
    environment = gym.envs.make("Breakout-v0")
    outputs = environment.action_space.n

    # Next, we create our networks:

    network = DQN(outputs, convolutional_layer_sizes, hidden_layer_sizes, scope="network")

    target_network = DQN(outputs, convolutional_layer_sizes, hidden_layer_sizes, scope="target_network")
    ...
    with tf.Session() as session:
        ...
        print("Total duration:", datetime.now() - start_time, file=open("output2.txt", "a"))
