...
class DSN:
    ...
    def save(self):
        ...
        np.savez('BCE_breakout_tensorflow_weights.npz', *parameters)

    def load(self):
        parameters = [variable for variable in tf.trainable_variables() if variable.name.startswith(self.scope)]
        npz = np.load('BCE_breakout_tensorflow_weights.npz')
        operations = []
...
def play_one_episode(environment, session, total_steps, experience_replay, network, target_network, image_transformer, gamma, batch_size, epsilon, epsilon_change, epsilon_minimum):
    ...
    while not done:
        if total_steps % copy_period == 0:
            ...
            print("Copied network parameters to target network. total steps = %s, copy period = %s" % (total_steps, copy_period),file=open("outputBCE.txt", "a"))
...
def main():
    ...
    with tf.Session() as session:
        ...
        print("Total duration:", datetime.now() - start_time, file=open(console_file, "a"))
