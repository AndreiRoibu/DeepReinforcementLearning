...
class DQN:
    ...
    def train(self, target_network):
        ...
        Q_predicted = np.max(target_network.predict(next_states), axis= 1)
...
def play_one_episode(environmnet, network, target_network, epsilon, gamma, copy_period, total_steps,penalty):
    ...
    while not done and iterations < 2000:
        ...
        action = network.sample_action(observation, epsilon)
        previous_observation = observation
        observation, reward, done, _ = environmnet.step(action)
        ...
        network.add_experience(previous_observation, action, reward, observation, done)
        network.train(target_network)
...
def play_evaluation_episode(environmnet, network, epsilon, penalty):
    ...
    while not done and iterations < 2000:
        ...
        observation, reward, done, _ = environmnet.step(environmnet.action_space.sample())
...
def main(console_file, learning_rate= 1e-4, decay= 0.99, gamma = 0.99, eps= 1e-3, copy_period=200, episodes=1000, penalty=0):
    ...
    network = DQN(inputs, outputs, hidden_layer_sizes, gamma, learning_rate, decay, eps)
    target_network = DQN(inputs, outputs, hidden_layer_sizes, gamma, learning_rate, decay, eps)
    ...
    for evaluation_episode in range(final_evalution_episodes):
        ...
        print('Episode:',evaluation_episode,' Reward:',evaluation_reward)
